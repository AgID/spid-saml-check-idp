...
class MainService {
	...
	*/
	...
	patchSAMLTest(testcase, hook, test, data, callback_response, callback_notfound, callback_error) {  
		Utility.log("PATCH /api/saml/report/" + testcase + "/" + hook + "/" + test);
		axios.patch(
			config.basepath + '/api/saml/report/' + testcase + '/' + hook + "/" + test + '?apikey=' + Utility.getApikey(),
			{data: data},
			{timeout: 900000}
		)
		.then(function(response) {
			Utility.log("patchSAMLTest Success", response.data);
			callback_response(response.data);
		})
		.catch(function(error) {
			Utility.log("patchSAMLTest Error", error);
			if(error.response.status==404) {
				Utility.log("patchSAMLTest Not Found");
				callback_notfound();
			} else {
				callback_error((error.response!=null) ? error.response.data : "Service not available");
			}
	...
	getAuthorizationRequest(testcase, callback_response, callback_error) {
		Utility.log("GET /api/saml/authrequest/" + testcase);
		axios.get(config.basepath + '/api/saml/authrequest/' + testcase + '?apikey=' + Utility.getApikey(), {timeout: 900000})
		.then(function(response) {
	...
	getReport(callback_response, callback_notfound, callback_error) {  
		Utility.log("GET /api/saml/report");
		axios.get(config.basepath + '/api/saml/report' + '?apikey=' + Utility.getApikey(), {timeout: 900000})
		.then(function(response) {
