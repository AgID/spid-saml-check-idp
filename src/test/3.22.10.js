...
class Test_3_22_10 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.10";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - the value of sid MUST be equal to 'saml:id' where id is the value of element ResponseID of the SAML Response";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.sid==null || grantTokenInnerSignedTokenPayload.sid=='') {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim sid is not present");
            }

            if(grantTokenInnerSignedTokenPayload.sid!='saml:' + this.authresponse.ResponseID) {
                this.notes = grantTokenInnerSignedTokenPayload.sid + " != " + "saml:" +  this.authresponse.ResponseID;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim sid is not equal to " + "saml:" +  this.authresponse.ResponseID);
            }            
        }
