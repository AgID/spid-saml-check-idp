...
class Test_3_22_8 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.8";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - the value of aud MUST be equal to the issuer of AA present on SPID OIDC Registry at the listing endpoint";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.aud==null || grantTokenInnerSignedTokenPayload.aud=='') {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim aud is not present");
            }
        }
