...
class Test_3_22_6 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.6";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - the value of iss MUST be equal to the EntityID of IdP";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.iss==null || grantTokenInnerSignedTokenPayload.iss=='') {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim iss is not present");
            }

            if(grantTokenInnerSignedTokenPayload.iss!=this.metadata.entity_id) {
                this.notes = grantTokenInnerSignedTokenPayload.iss + " != " + this.metadata.entity_id;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim iss is not equal to the EntityID of IdP");
            }
        }
