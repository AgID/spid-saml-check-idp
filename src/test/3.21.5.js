...
class Test_3_21_5 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.21.5";
        this.description = "GrantToken JWE - the value of typ MUST be 'aa-grant+jwt'";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(jwe.header.typ==null || jwe.header.typ=='') {
                ...
                throw("GrantToken JWE claim typ is not present");
            }

            if(jwe.header.typ!='aa-grant+jwt') {
                this.notes = jwe.header;
                throw("GrantToken JWE claim typ is not 'aa-grant+jwt'");
            }            
        }
