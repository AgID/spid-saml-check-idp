...
class Test_3_22_23 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.23";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - act object MUST contain claim sub";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.act.sub==null || grantTokenInnerSignedTokenPayload.act.sub=='') {
                this.notes = grantTokenInnerSignedTokenPayload.act;
                throw("GrantToken JWS (Grant Token Inner Signed Token) act object does not contain claim sub");
            }
        }
