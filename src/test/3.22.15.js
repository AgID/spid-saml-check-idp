...
class Test_3_22_15 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.15";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - the value of iat MUST be < current date + 3min";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.iat==null || grantTokenInnerSignedTokenPayload.iat=='') {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim iat is not present");
            }

            if (!moment.unix(grantTokenInnerSignedTokenPayload.iat).isValid()) {
                this.notes = grantTokenInnerSignedTokenPayload.iat;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim iat is not a valid unix time");
            }
            ...
            if(!moment.unix(grantTokenInnerSignedTokenPayload.iat).isBefore(moment().add(3, 'm'))) {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim iat is > current date + 3min");
            }            
        }
