...
class Test_3_22_1 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.1";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - the value of alg MUST be one of ['RS256', 'RS512']";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(jws.header.alg==null || jws.header.alg=='') {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim alg is not present");
            }

            if(!['RS256', 'RS512'].includes(jws.header.alg)) {
                this.notes = jws.header;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim alg is not one of ['RS256', 'RS512']");
            }
        }
