...
class Test_3_22_20 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.20";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - claim sub MUST be present";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.sub==null || grantTokenInnerSignedTokenPayload.sub=='') {
                this.notes = grantTokenInnerSignedTokenPayload.sub;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim sub is not present");
            }
        }
