...
class Test_3_22_19 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.19";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - if present, the value of nbf MUST be a valid unix time";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.nbf!=null && !moment.unix(grantTokenInnerSignedTokenPayload.nbf).isValid()) {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) if present, claim nbf is not a valid unix time");
            }
        }
