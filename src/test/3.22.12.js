...
class Test_3_22_12 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.12";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - claim acr MUST be one of [https://www.spid.gov.it/SpidL1, https://www.spid.gov.it/SpidL2, https://www.spid.gov.it/SpidL3] and MUST be the same of SPID level used for auth";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.acr==null || grantTokenInnerSignedTokenPayload.acr=='') {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim acr is not present");
            }
            ...
            if(!['https://www.spid.gov.it/SpidL1', 'https://www.spid.gov.it/SpidL2', 'https://www.spid.gov.it/SpidL3'].includes(grantTokenInnerSignedTokenPayload.acr)) {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim acr is not one of [https://www.spid.gov.it/SpidL1, https://www.spid.gov.it/SpidL2, https://www.spid.gov.it/SpidL3]");
            }

            if(grantTokenInnerSignedTokenPayload.acr!=this.authrequest.AuthnContextClassRef) {
                this.notes = grantTokenInnerSignedTokenPayload.acr;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim acr is not the same of SPID level used for auth (" + this.authrequest.AuthnContextClassRef + ")");
            }
        }
