...
class Test_3_22_17 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.17";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - the value of exp MUST be a valid unix time";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(grantTokenInnerSignedTokenPayload.exp==null || grantTokenInnerSignedTokenPayload.exp=='') {
                ...
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim exp is not present");
            }

            if (!moment.unix(grantTokenInnerSignedTokenPayload.exp).isValid()) {
                this.notes = grantTokenInnerSignedTokenPayload.exp;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim exp is not a valid unix time");
            }         
        }
