...
class Test_3_21_1 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.21.1";
        this.description = "GrantToken JWE - the value of alg MUST be one of ['RSA-OAEP', 'RSA-OAEP-256']";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(jwe.header.alg==null || jwe.header.alg=='') {
                ...
                throw("GrantToken JWE claim alg is not present");
            }

            if(!['RSA-OAEP', 'RSA-OAEP-256'].includes(jwe.header.alg)) {
                this.notes = jwe.header;
                throw("GrantToken JWE claim alg is not one of ['RSA-OAEP', 'RSA-OAEP-256']");
            }            
        }
