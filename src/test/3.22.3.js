...
class Test_3_22_3 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.3";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - the signature of Grant Token Inner Signed Token MUST be able to be verified with the public key of the IdP";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(!jws) {
                this.notes = jws;
                throw("the signature of Grant Token Inner Signed Token cannot be verified with the public key of the IdP");
            }     
        }
