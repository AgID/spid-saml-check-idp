...
class Test_3_22_24 extends TestAuthResponse {

    constructor(metadata, authrequest, authresponse) {
        super(metadata, authrequest, authresponse);
        this.num = "3.22.24";
        this.description = "GrantToken JWS (Grant Token Inner Signed Token) - claim sub of the act object MUST be equal to the EntityID of SP";
    ...
    async exec() {
        ...
        for(let t in Extensions.GrantedAttributeAuthority[0]) {
            ...
            if(!(grantTokenInnerSignedTokenPayload.act===Object(grantTokenInnerSignedTokenPayload.act))) {
                this.notes = grantTokenInnerSignedTokenPayload.act;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim act is not a valid json object");                
            }
            
            if(grantTokenInnerSignedTokenPayload.act.sub!=config_sp.entity_id) {
                this.notes = grantTokenInnerSignedTokenPayload.act.sub + " != " + config_sp.entity_id;
                throw("GrantToken JWS (Grant Token Inner Signed Token) claim sub of the act object is not equal to the EntityID of SP: " + config_sp.entity_id);
            }
        }
